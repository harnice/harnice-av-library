"""Emit Neutrik cable-connector SKUs as Harnice parts.

The catalog, legality filter and part-number grammar live in neutrik.py.
...
LIBRARY_REPO = neutrik.LIBRARY_REPO
LIBRARY_SUBPATH = neutrik.LIBRARY_SUBPATH
MANUFACTURER = neutrik.MANUFACTURER
HOUSING_NAMES = neutrik.HOUSING_NAMES
...
# ---------------------------------------------------------------------------
def profile_for(cfg):
    """Attach the family envelope. Catalog identity lives in neutrik.py."""
    family = cfg["family"]
    if family == "XLR":
        return xlr_xx_profile(cfg["gender"])
    if family == "etherCON":
        return ethercon_profile()
    if family == "powerCON":
        if cfg.get("true1") or "TRUE1" in cfg["series"]:
            return powercon_true1_profile(large_cable=cfg["mpn"].endswith("-L"))
        return powercon_classic_profile()
    if family == "TRS":
        return phone_plug_profile(
            cfg["poles"],
            handle_dia=cfg["handle_dia"],
            bushing_dia=cfg["bushing_dia"],
        )
    if family == "speakON":
        return speakon_profile(cfg["body_dia"], cfg["total_length"], cfg["bushing_dia"])
    raise ValueError(f"No envelope for family {family!r}")


def iter_variants(families=None):
    for cfg in neutrik.iter_part_configurations(families):
        variant = dict(cfg)
        variant["profile"] = profile_for(variant)
...
def part_description(variant):
    return neutrik.description_from_cfg(variant)
...
def write_catalog_csv(path=None):
    if path is None:
        path = os.path.join(_FAMILY_DIR, "neutrik.csv")
    rows = [
        neutrik.catalog_row(cfg) for cfg in neutrik.iter_part_configurations()
    ]
    with open(path, "w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=neutrik.CATALOG_COLUMNS, lineterminator="\n"
        )
        writer.writeheader()
...
def build_part(part_number, rev_dir):
    """Run the harnice part build in this process (same as `harnice -b`)."""
    cwd = os.getcwd()
    os.chdir(rev_dir)
    try:
